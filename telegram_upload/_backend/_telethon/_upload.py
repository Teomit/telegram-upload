...
from telethon.errors import FloodWaitError, InvalidBufferError, RPCError
...
class TelegramUploadClient(TelegramClient):
    ...
    def send_one_file(self, entity, file: File, send_as_media: bool = False, thumb: str | None = None,
                      retries=RETRIES):
        message = None
    ...
    def send_files(self, entity, files: Iterable[File], delete_on_success=False, print_file_id=False,
        ...
        for file in files:
            ...
            try:
                ...
                if thumb and not file.is_custom_thumbnail and os.path.lexists(thumb):
                    try:
                        ...
                        click.echo(f'Warning: Failed to delete thumbnail "{thumb}": {e}', err=True)
            ...
            if print_file_id:
                click.echo(f'Uploaded successfully "{file.file_name}" (file_id {pack_bot_file_id(message.media)})')
    ...
    async def upload_file(
        ...
        async with helpers._FileStream(file, file_size=file_size) as stream:
            ...
            await asyncio.wait([
                task for task in asyncio.all_tasks() if task.get_name().startswith("telegram-upload-file-")
            ])
        if is_big:
